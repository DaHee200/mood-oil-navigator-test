...
export function HomePageClient() {
  ...
  useEffect(() => {
    ...
    runWithMDC({ traceId, path: '/' }, () => {
      logHttp(200, `Main Landing Page loaded with A/B Variant [${assignedVariant}]`, {
  ...
  const switchVariant = (newVariant: 'A' | 'B') => {
    ...
    const traceId = generateTraceId();
    runWithMDC({ traceId, path: '/' }, () => {
      logHttp(200, `User manually switched A/B test variant to [${newVariant}]`, {
        variant: newVariant,
      });
    });
  };
  ...
  return (
    <>
      {variant === 'A' ? (
        /* Variant A: Minimalist Warm Botanical Mood 🌿 */
        <main className="flex min-h-screen w-full flex-col items-center justify-center bg-[#F2F7F2] p-4 relative">
          <Card className="w-full max-w-xl animate-fade-in text-center shadow-lg bg-emerald-50/90 border-emerald-200/80 rounded-2xl p-6 sm:p-8">
            <CardHeader className="space-y-2">
              <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 shadow-inner">
                <Leaf className="h-10 w-10" />
              </div>
              <CardTitle className="font-headline text-5xl font-extrabold tracking-tight text-emerald-950">
                Find your Oils
              </CardTitle>
              <CardDescription className="pt-2 text-xl my-10 text-emerald-800/80 font-body">
                당신의 오늘 분위기에 맞는 오일을 추천해드려요!
              </CardDescription>
            </CardHeader>
            <CardContent className="py-6">
              <Button asChild size="lg" className="w-full max-w-xs py-7 text-xl font-bold bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl shadow-md">
                <Link href="/quiz?variant=A">시작하기</Link>
              </Button>
            </CardContent>
            <CardFooter className="flex justify-center pb-6 pt-2">
              <p className="text-xs sm:text-sm font-medium text-red-600/90 bg-red-50/80 px-4 py-2 rounded-lg border border-red-200/60">
        ...
        /* Variant B: Vibrant Pastel Counselor Mood ✨ */
      ...
      <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 bg-slate-900/90 text-white px-3.5 py-2.5 rounded-full shadow-2xl text-xs font-sans border border-slate-700 backdrop-blur-md">
        ...
        <span className="px-2.5 py-0.5 rounded-md bg-slate-800 text-emerald-300 font-extrabold">
          {variant === 'A' ? 'A안 (보타니컬 내추럴)' : 'B안 (파스텔 카운슬링)'}
        </span>
        <button
