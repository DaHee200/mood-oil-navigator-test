...
const QuizClient: FC = () => {
  ...
  const handleSubmit = async (finalAnswers: string[]) => {
    ...
    runWithMDC({ traceId, path: '/quiz' }, async () => {
      try {
        const result = await getOilRecommendation(finalAnswers, traceId);

        if (result.success && result.data) {
          setRecommendation(result.data);
          logHttp(200, 'Recommendation successfully retrieved on client', { traceId });
        } else {
          logHttp(400, `Client received server error: ${result.error}`, { traceId, error: result.error });
          toast({
            variant: 'destructive',
            title: '오류',
        ...
        logHttp(500, 'Client communication exception during server action call', { traceId }, error);
        toast({
          variant: 'destructive',
          title: '연결 오류',
  ...
  if (isLoading) {
    return (
      <main className={`flex min-h-screen w-full items-center justify-center p-4 ${variant === 'A' ? 'bg-[#F2F7F2]' : 'bg-gradient-to-b from-secondary/20 via-background to-primary/10'}`}>
        <Card className={`w-full max-w-xl text-center shadow-xl ${variant === 'A' ? 'bg-emerald-50/90 border-emerald-200/80 rounded-2xl' : 'border-primary/20 bg-white/90 backdrop-blur-md rounded-3xl'}`}>
          <CardHeader>
            <CardTitle className={`font-headline text-4xl font-bold ${variant === 'A' ? 'text-emerald-900' : 'text-primary'}`}>
              {variant === 'A' ? '오일 찾는 중...' : '나만의 오일 찾는 중...'}
            </CardTitle>
            <CardDescription className="text-lg">
              {variant === 'A' ? '당신의 상태에 맞는 최적의 오일을 선별하고 있습니다.' : '당신의 기분과 고민을 다정하게 분석하고 있습니다.'}
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col items-center justify-center space-y-6 py-16">
            <Loader2 className={`h-20 w-20 animate-spin ${variant === 'A' ? 'text-emerald-700' : 'text-primary'}`} />
  ...
  if (recommendation) {
    ...
    if (variant === 'A') {
      /* VARIANT A RESULT: Minimalist Classic Card (Direct Reveal, Natural Green Theme, Includes Warning Notice) */
      return (
        <main className="flex min-h-screen w-full flex-col items-center justify-center bg-[#F2F7F2] p-4 py-8 sm:py-12">
          <Card className="w-full max-w-2xl animate-fade-in text-center shadow-lg bg-emerald-50/90 border-emerald-200/80 rounded-2xl overflow-hidden">
            <CardHeader className="border-b border-emerald-200/60 pb-6 bg-emerald-100/50">
              <CardTitle className="font-headline text-4xl text-emerald-950 flex items-center justify-center gap-2">
                <Leaf className="h-8 w-8 text-emerald-700" />
                오늘의 향기
              </CardTitle>
              <CardDescription className="text-lg text-emerald-800/80 font-body">당신의 상태를 분석하여 찾은 맞춤 에센셜 오일입니다.</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col items-center space-y-8 p-6 md:p-8">
              
              <div className="flex flex-col md:flex-row items-center gap-8 w-full">
                <div className="relative h-64 w-64 bg-white rounded-2xl p-4 shadow-md flex-shrink-0 border border-emerald-200/60">
                  <Image
                    src={oil.image}
                    alt={`${oil.name} oil`}
                    fill
                ...
                <div className="text-left space-y-4 flex-grow">
                  <div>
                    <span className="text-sm font-semibold text-emerald-800 px-3 py-1 bg-emerald-100 rounded-full border border-emerald-200">🌿 추천 오일</span>
                    <h3 className="font-headline text-4xl font-extrabold text-emerald-950 mt-2">{oil.name}</h3>
                    <p className="text-lg text-emerald-700/80 capitalize mt-1">({oil.id.replace('-', ' ')})</p>
                  </div>
                  <p className="text-base text-emerald-900/90 border-l-2 border-emerald-500 pl-3 italic font-body">
                    {oil.description}
                  </p>
                </div>
              </div>
              ...
              <div className="w-full space-y-6 text-left">
                {sections.messageForYou && (
                  <div className="bg-white/90 backdrop-blur-sm p-5 rounded-2xl border border-emerald-200/60 shadow-sm">
                    <h4 className="font-semibold text-lg flex items-center gap-2 text-emerald-950 mb-2">
                      <span>💭</span> 지금의 당신에게
                    </h4>
                    <p className="text-base text-emerald-900/90 leading-relaxed font-body whitespace-pre-line">
                      {sections.messageForYou}
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {sections.scentFeel && (
                    <div className="bg-emerald-100/40 p-5 rounded-2xl border border-emerald-200/60 shadow-sm flex flex-col justify-between">
                      <h4 className="font-semibold text-lg flex items-center gap-2 text-emerald-950 mb-2">
                        <span>🌸</span> 향의 느낌
                      </h4>
                      <p className="text-base text-emerald-900/90 leading-relaxed flex-grow whitespace-pre-line font-body">
                        {sections.scentFeel}
                      </p>
                    </div>
                  )}
            ...
            <CardFooter className="flex flex-col gap-4 p-6 border-t border-emerald-200/60 bg-emerald-100/30">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 w-full">
                <Button
                  size="lg"
                  variant="outline"
                  className="w-full text-base border-emerald-300 text-emerald-900 hover:bg-emerald-100"
                  onClick={() => router.push(`/oils/${encodeURIComponent(oil.id)}`)}
                >
                  <Sparkles className="mr-2 h-5 w-5 text-emerald-700" /> 상세 정보
                </Button>
                <Button asChild size="lg" className="w-full text-base font-bold bg-emerald-700 hover:bg-emerald-800 text-white">
              ...
              {/* Variant A Footer Warning Notice */}
              <p className="text-xs sm:text-sm font-medium text-red-600/90 bg-red-50/80 px-4 py-2 rounded-lg border border-red-200/60 w-full text-center">
                * 5개월 미만의 임산부에게는 아로마 오일 사용을 권장하지 않습니다.
              </p>
            </CardFooter>
          </Card>
        </main>
    ...
    /* VARIANT B RESULT: Vibrant Pastel Counselor Layout with 3D Card Flip & Trust Badge (No Warning Notice) */
    return (
  ...
  return (
    <main className={`flex min-h-screen w-full items-center justify-center p-4 ${variant === 'A' ? 'bg-[#F2F7F2]' : 'bg-gradient-to-b from-secondary/20 via-background to-primary/10'}`}>
      <Card className={`w-full max-w-2xl shadow-xl ${variant === 'A' ? 'bg-emerald-50/90 border-emerald-200/80 rounded-2xl' : 'border-primary/10 rounded-3xl bg-white/95'}`}>
        <CardHeader>
          <Progress value={progressValue} className={`mb-6 h-2.5 ${variant === 'A' ? 'bg-emerald-200/60' : 'bg-secondary/30'}`} />
          <CardTitle className={`font-headline text-3xl sm:text-4xl font-bold ${variant === 'A' ? 'text-emerald-950' : 'text-foreground'}`}>기분 퀴즈</CardTitle>
          <CardDescription className={`text-lg ${variant === 'A' ? 'text-emerald-800/80' : 'text-muted-foreground'}`}>
        ...
        <CardContent className={`min-h-[350px] transition-opacity duration-300 ${isFading ? 'opacity-0' : 'opacity-100'}`}>
          {currentQuestion && (
            <div className="flex flex-col items-center space-y-6 text-center">
              <div className={`p-4 rounded-full shadow-sm ${variant === 'A' ? 'bg-emerald-100 text-emerald-800' : 'bg-primary/10 text-primary'}`}>
                <currentQuestion.icon className="h-12 w-12 sm:h-14 sm:w-14" />
              </div>
              <p className={`text-2xl font-bold ${variant === 'A' ? 'text-emerald-950 font-body' : 'text-foreground'}`}>{currentQuestion.question}</p>
        ...
        <CardFooter className="flex justify-between items-center">
          <Button variant="outline" onClick={handleBack} className={`rounded-xl ${variant === 'A' ? 'border-emerald-300 text-emerald-950 hover:bg-emerald-100' : ''}`}>
            <ChevronLeft className="mr-2 h-4 w-4" /> 뒤로
          </Button>
          {variant === 'A' && (
            <p className="text-xs font-medium text-red-600/90 bg-red-50/80 px-3 py-1.5 rounded-lg border border-red-200/60">
              * 5개월 미만 임산부 주의
            </p>
          )}
        </CardFooter>
