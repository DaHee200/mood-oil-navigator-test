...
const QuizClient: FC = () => {
  ...
  if (isLoading) {
    return (
      <Card className="w-full max-w-xl text-center shadow-xl border-primary/20 bg-white/90 backdrop-blur-md">
        <CardHeader>
          <CardTitle className="font-headline text-4xl text-primary font-bold">나만의 오일 찾는 중...</CardTitle>
          <CardDescription className="text-lg">당신의 기분과 고민을 다정하게 분석하고 있습니다.</CardDescription>
        </CardHeader>
  ...
  return (
    <Card className="w-full max-w-2xl shadow-xl border-primary/10 rounded-3xl">
      <CardHeader>
        <Progress value={progressValue} className="mb-6 h-2.5 bg-secondary/30" />
        <CardTitle className="font-headline text-3xl sm:text-4xl text-foreground font-bold">기분 퀴즈</CardTitle>
      ...
      <CardContent className={`min-h-[350px] transition-opacity duration-300 ${isFading ? 'opacity-0' : 'opacity-100'}`}>
        {currentQuestion && (
          <div className="flex flex-col items-center space-y-6 text-center">
            <div className="p-4 bg-primary/10 rounded-full text-primary shadow-sm">
            ...
            <div className="grid w-full grid-cols-2 justify-center gap-3 pt-4 sm:grid-cols-3 md:grid-cols-4">
              {currentQuestion.options.map((option) => (
                <Button 
                  ...
                  size="lg" 
                  className={`w-full h-auto px-4 py-3.5 text-base font-bold shadow-sm hover:scale-105 hover:shadow-md transition-all duration-200 border rounded-2xl whitespace-normal ${option.color}`}
                >
                  {option.text}
                </Button>
              ))}
